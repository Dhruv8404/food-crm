...
import { ChefHat, Clock, CheckCircle, AlertTriangle, Users, TrendingUp, Timer, Package, Sparkles, RefreshCw } from "lucide-react"
...
export default function ChefDashboard() {
  ...
  const completedOrders = pendingOrders.filter(order => order.status === 'completed')
  const customerPaidOrders = orders.filter(order => order.status === 'customer_paid')

  const totalItems = pendingOrders.reduce((total, order) => 
        ...
        >
          ...
          <Card className="border-0 shadow-2xl bg-white/80 backdrop-blur-sm overflow-hidden">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-purple-600 mb-1">Paid Online</p>
                  <p className="text-3xl font-bold text-gray-900">{customerPaidOrders.length}</p>
              ...
              <CardContent className="p-4 space-y-4">
                <AnimatePresence mode="popLayout">
                  {customerPaidOrders.map((order) => (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -20 }}
                      layout
                      className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-4 border-2 border-purple-200 shadow-sm hover:shadow-md transition-all duration-300"
                    >
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          <h3 className="font-bold text-gray-900 text-lg">Order #{order.id.slice(-6)}</h3>
                      ...
                      <div className="bg-purple-100 border border-purple-200 rounded-lg p-3 mb-3">
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-semibold text-purple-800">Paid Online</span>
                          <CheckCircle className="w-4 h-4 text-purple-600" />
                        </div>
                        <p className="text-xs text-purple-600 mt-1">
                          Payment received digitally
                        </p>
                      </div>
                ...
                {customerPaidOrders.length === 0 && (
                  ...
                  >
                    ...
                    <h3 className="font-semibold text-gray-600 mb-2">No Online Payments</h3>
                    <p className="text-sm">All payments at counter</p>
                  </motion.div>
                )}
              </CardContent>
            </Card>
